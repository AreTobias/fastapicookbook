...
DB_FILENAME = os.path.join(os.path.dirname(__file__), "tasks.csv")
...
def write_task_into_csv(task: TaskWithID):
    with open(DB_FILENAME, mode="a", newline="") as file:
        ...
        write.writerow(task.model_dump())
...
def modify_task(id: int, task: dict) -> Optional[TaskWithID]:
    ...
    with open(DB_FILENAME, mode="w", newline="") as csvfile:
        ...
        writer.writeheader()
        for task in tasks:
            writer.writerow(tasks.model_dump())
...
def create_test_database():
    database_file_location = str(Path(__file__).parent / TEST_DB_FILE)
    with patch(
        "operations.DB_FILENAME",
